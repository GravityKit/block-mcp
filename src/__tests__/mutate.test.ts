...
describe('handleMutateTool', () => {
  ...
  it('throws on missing post_id', async () => {
    await expect(
      handleMutateTool('edit_block_tree', { op: 'update-attrs', path: [0], attributes: {} }, mockClient)
    ).rejects.toThrow('post_id is required');
  });

  it('throws on invalid op', async () => {
    await expect(
      handleMutateTool('edit_block_tree', { post_id: 1, op: 'fake', path: [0] }, mockClient)
    ).rejects.toThrow('op must be one of');
  });

  it('throws on non-array path', async () => {
    await expect(
      handleMutateTool('edit_block_tree', { post_id: 1, op: 'update-attrs', path: 'not-array', attributes: {} }, mockClient)
    ).rejects.toThrow('must be an array of integers');
  });

  it('throws on path with non-integers', async () => {
    await expect(
      handleMutateTool('edit_block_tree', { post_id: 1, op: 'update-attrs', path: [0, 'abc', 2], attributes: {} }, mockClient)
  ...
  it('update-attrs requires attributes', async () => {
    await expect(
      handleMutateTool('edit_block_tree', { post_id: 1, op: 'update-attrs', path: [0] }, mockClient)
    ).rejects.toThrow('attributes');
  });

  it('update-html requires innerHTML', async () => {
    await expect(
      handleMutateTool('edit_block_tree', { post_id: 1, op: 'update-html', path: [0] }, mockClient)
    ).rejects.toThrow('innerHTML');
  });

  it('replace-block requires block with name', async () => {
    await expect(
      handleMutateTool('edit_block_tree', { post_id: 1, op: 'replace-block', path: [0], block: {} }, mockClient)
  ...
  it('insert-child requires block with name', async () => {
    await expect(
      handleMutateTool('edit_block_tree', { post_id: 1, op: 'insert-child', path: [0] }, mockClient)
