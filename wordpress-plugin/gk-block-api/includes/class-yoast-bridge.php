 ...
 * register when WPSEO_FILE is defined, so a clean install of gk-block-api
 * without Yoast SEO contributes zero routes.
 */
class Yoast_Bridge {

	/**
	 * REST namespace shared with the rest of gk-block-api.
	 */
	...
	public function get_seo( \WP_REST_Request $request ) {
		...
		if ( ! get_post( $post_id ) ) {
			return new \WP_Error( 'not_found', __( 'Post not found.', 'gk-block-api' ), array( 'status' => 404 ) );
		}
	...
	public function update_seo( \WP_REST_Request $request ) {
		...
		if ( ! is_array( $params ) || empty( $params ) ) {
			return new \WP_Error(
				'invalid_body',
				__( 'Request body must be a JSON object with fields to update.', 'gk-block-api' ),
				array( 'status' => 400 )
			);
		}
	...
	public function bulk_update_seo( \WP_REST_Request $request ) {
		...
		foreach ( $posts as $entry ) {
			if ( ! is_array( $entry ) || empty( $entry['post_id'] ) ) {
				$results[] = array( 'error' => __( 'Missing post_id.', 'gk-block-api' ) );
				continue;
			}
			...
			if ( ! current_user_can( 'edit_post', $post_id ) ) {
				$results[] = array(
					'post_id' => $post_id,
					'error'   => __( 'Permission denied.', 'gk-block-api' ),
				);
				continue;
			}
			...
			if ( ! get_post( $post_id ) ) {
				$results[] = array(
					'post_id' => $post_id,
					'error'   => __( 'Post not found.', 'gk-block-api' ),
				);
				continue;
			}
