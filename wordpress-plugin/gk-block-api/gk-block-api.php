 ...
 *
 * When an agent connection is live at deactivation time — the service
 * account exists and holds at least one Application Password — an admin
 * notice is queued so the site owner is given a clear path to revoke the
...
function on_deactivation() {
	...
	// Queue a 5-minute notice; the deactivated plugin's admin_notices hook
	// is no longer active, so we rely on a first-load from another page.
...
/**
 * Show the deactivation-notice banner and wire the revoke-all action.
 *
 * The banner is shown once — the transient is deleted on first display.
...
function init_deactivation_notice() {
	...
	add_action( 'admin_post_gk_block_api_revoke_all', __NAMESPACE__ . '\\handle_revoke_all' );
}
add_action( 'admin_init', __NAMESPACE__ . '\\init_deactivation_notice' );
...
function maybe_show_deactivation_notice() {
	...
	$revoke_url = wp_nonce_url(
		admin_url( 'admin-post.php?action=gk_block_api_revoke_all' ),
		'gk_block_api_revoke_all'
	);
	?>
	<div class="notice notice-warning is-dismissible">
		<p>
			<strong><?php esc_html_e( 'GK Block API deactivated — active AI connections remain.', 'gk-block-api' ); ?></strong>
		</p>
		<p>
			<?php
			echo wp_kses(
				sprintf(
					/* translators: %s: URL of the revoke-all action */
					__( 'The Block MCP service account and its Application Passwords are still present. <a href="%s">Revoke all connections now</a> to remove them, or reactivate the plugin to manage them from the Connect page.', 'gk-block-api' ),
					esc_url( $revoke_url )
				),
				array(
					'a' => array( 'href' => array() ),
...
function handle_revoke_all() {
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_die(
			esc_html__( 'You do not have permission to do this.', 'gk-block-api' ),
			'',
			array( 'response' => 403 )
		);
	}
