...
class Block_CRUD {
		...
		if ( $depth > self::MAX_BLOCK_DEPTH ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Block tree exceeds the maximum nesting depth (%1$d levels, got %2$d).', 'gk-block-mcp' ),
	...
	public function resolve_ref( $post_id, $ref ) {
		if ( ! is_string( $ref ) || '' === $ref ) {
			return new \WP_Error( 'invalid_ref', __( 'Ref must be a non-empty string.', 'gk-block-mcp' ), array( 'status' => 400 ) );
		}
		...
		if ( null === $path ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Ref "%s" not found. The block may have been deleted, or the ref is from an older snapshot. Re-fetch the page to get current refs.', 'gk-block-mcp' ),
					$ref
				),
				array(
					'status' => 404,
	...
	public function resolve_ref_to_index( $post_id, $ref ) {
		...
		return new \WP_Error(
			...
			sprintf(
				/* translators: %s: ref string */
				__( 'Ref "%s" not found.', 'gk-block-mcp' ),
				$ref
			),
			array(
				'status' => 404,
	...
	public function get_block( $post_id, $ref = null, $flat_index = null ) {
		...
		if ( $has_ref ) {
			...
			if ( $flat_idx < 0 || $flat_idx >= count( $flat ) ) {
				return new \WP_Error(
					'invalid_index',
					__( 'flat_index out of range.', 'gk-block-mcp' ),
					array( 'status' => 400 )
				);
			}
		}
		...
		if ( null === $block || ! is_array( $block ) ) {
			return new \WP_Error(
				'block_not_found',
				__( 'Could not resolve block at the computed path.', 'gk-block-mcp' ),
				array( 'status' => 404 )
			);
		}
	...
	public function resolve_ref_to_top_level( $post_id, $ref ) {
		...
		if ( count( $path ) !== 1 ) {
			return new \WP_Error(
				'ref_not_top_level',
				__( 'Ref refers to a nested block; insert_blocks before/after_ref requires a top-level block.', 'gk-block-mcp' ),
				array(
					'status' => 400,
					'ref'    => $ref,
					'path'   => $path,
		...
		return new \WP_Error( 'ref_stale', __( 'Ref position could not be resolved.', 'gk-block-mcp' ), array( 'status' => 404 ) );
	}
