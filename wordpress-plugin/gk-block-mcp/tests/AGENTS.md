# AGENTS.md — gk-block-mcp PHPUnit suite
...
- **Authorship tests**: connection meta no longer drives authorship (the byline subsystem / `author_to_credit()` was removed in 2.0), so there is no UUID-global simulation. The relevant coverage is the provision-identity tests in `ConnectPageTest` (`test_provision_credentials_agent_mints_on_agent`, `test_provision_credentials_self_mints_on_current_user`, `test_provision_credentials_unknown_identity_falls_back_to_agent`, `test_self_identity_disabled_by_filter_clamps_to_agent`) and the author-assignment tests in `PostManagerTest` (`test_create_post_does_not_remap_author_from_connection_meta`, `test_create_post_honors_explicit_author_argument`).
...
- **Ignore intelephense's "Undefined method `assertX`" on `WP_UnitTestCase` descendants.** The phpunit-polyfills `class_alias` chain defeats the IDE's static resolution, so it flags every assertion method as undefined while the suite runs green. `composer test` is the truth; don't "fix" these diagnostics. (Same goes for the occasional "Expected type 'WP_Error'. Found 'int'." on `int|WP_Error` returns: PHPStan is the authority, and it stays `[OK]`.)
- **`composer test | tail` can report a misleadingly tiny test count.** A tailed `composer test` once showed "OK (5 tests)" while the real suite is 1300+ (the wrapper's pre-step output masked the actual run). For the true total, run the config directly: `vendor/bin/phpunit -c tests/phpunit.xml 2>&1 | grep -E 'OK \(|Tests: '`. [2026-07]
- **Per-file `vendor/bin/phpcs <file>` hides alignment WARNINGS that CI fails on; run `composer lint` before pushing.** phpcs exits 0 when a file has only warnings (no errors), so a per-file check reads as "clean" while `composer lint` (the CI gate, full tree) fails. Two warning/error types bit a 2.1.0 refactor: `Generic.Formatting.MultipleStatementAlignment` (shortening one value in an aligned `=` block misaligns the run; `composer lint:fix` fixes it), and a `WordPress-Docs` error when a docblock long description starts lowercase (it must begin with a capital). Run `composer lint`, not per-file phpcs, and don't trust the exit code alone. [2026-07]
